...
function NavItem({ label, href }: { label: string; href: string }) {
  return (
    <a
      href={href}
      className="text-sm font-medium text-[#475569] hover:text-[#14532D] transition-colors relative group"
    >
      {label}
      <span className="absolute -bottom-1 left-0 h-0.5 w-0 bg-[#16A34A] rounded-full transition-all group-hover:w-full" />
    </a>
  );
}
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div className="h-12 w-12 rounded-xl bg-emerald-50 text-emerald-700 flex items-center justify-center group-hover:bg-emerald-100 transition-colors">
          {icon}
        </div>
      ...
      <div className="mt-auto flex items-center text-xs font-semibold text-emerald-700 gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-sans">
      ...
      >
        <div className="container-wide flex items-center justify-between h-16">
          ...
          {/* Mobile menu toggle */}
          <button
            className="md:hidden p-2 text-[#475569] hover:text-[#0F172A] cursor-pointer"
        ...
        {/* Mobile dropdown */}
        {menuOpen && (
          <div className="md:hidden bg-white border-t border-[#E2E8F0] px-6 py-4 flex flex-col gap-4 shadow-lg animate-fade-in">
            {['Sobre', 'Módulos', 'Segurança', 'Contato'].map((item) => (
              <a
                key={item}
                href={`#${item.toLowerCase()}`}
                className="text-sm font-medium text-[#475569] hover:text-[#14532D] py-1"
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="absolute bottom-0 -left-32 w-[400px] h-[400px] rounded-full bg-emerald-400/10 blur-3xl" />
        ...
        <div className="container-wide relative z-10 pt-24 pb-16">
          <div className="max-w-3xl">
            {/* Pill badge */}
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 border border-white/20 text-emerald-200 text-xs font-semibold mb-8 animate-fade-in-up">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white font-display leading-tight mb-6 animate-fade-in-up delay-100">
              ...
              <span className="block text-emerald-300 mt-1">operações pecuárias</span>
            </h1>

            <p className="text-lg text-emerald-100 leading-relaxed max-w-xl mb-10 animate-fade-in-up delay-200">
            ...
            <div className="flex flex-col sm:flex-row gap-4 animate-fade-in-up delay-300">
              <button
                id="btn-hero-access"
                onClick={onNavigateToLogin}
                className="inline-flex items-center justify-center gap-2 px-8 py-3.5 bg-white text-[#14532D] font-bold rounded-full text-base shadow-xl hover:shadow-2xl hover:-translate-y-0.5 transition-all cursor-pointer"
              >
                Acessar o Sistema
              ...
              <a
                href="#modulos"
                className="inline-flex items-center justify-center gap-2 px-8 py-3.5 bg-transparent border-2 border-white/40 text-white font-semibold rounded-full text-base hover:bg-white/10 hover:border-white/60 transition-all cursor-pointer"
              >
                Ver Módulos
            ...
            <div className="flex flex-wrap items-center gap-6 mt-12 animate-fade-in-up delay-400">
              {['Rastreabilidade GTA', 'Integração SEFAZ', 'SSL-256', 'LGPD'].map((tag) => (
                <div key={tag} className="flex items-center gap-1.5 text-emerald-200 text-xs font-medium">
      ...
      <section id="sobre" className="section-padding bg-white">
        <div className="container-wide">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            {/* Text */}
            <div>
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-xs font-semibold text-emerald-700 uppercase tracking-wide mb-6">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
                Sobre o Sistema
              </span>
              <h2 className="text-3xl sm:text-4xl font-bold text-[#0F172A] font-display leading-tight mb-6">
                  ...
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between py-3 border-b border-[#E2E8F0] last:border-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <TrendingUp className={`h-3.5 w-3.5 ${item.up ? 'text-emerald-500' : 'text-amber-500'}`} />
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <section className="section-padding bg-[#F8FAFC]">
        <div className="container-wide">
          <div className="text-center mb-12">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-xs font-semibold text-emerald-700 uppercase tracking-wide mb-4">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
              Nossa Essência
            </span>
            <h2 className="text-3xl font-bold text-[#0F172A] font-display">Missão, Visão & Valores</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {MVP_VALUES.map((item, i) => (
              ...
              >
                <div className="h-14 w-14 rounded-2xl bg-emerald-50 text-emerald-700 flex items-center justify-center group-hover:bg-emerald-100 transition-colors">
                  {item.icon}
                </div>
      ...
      >
        <div className="container-wide">
          <div className="text-center mb-12">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 border border-white/20 text-emerald-300 text-xs font-semibold uppercase tracking-wide mb-4">
            ...
            <p className="text-emerald-200 max-w-xl mx-auto leading-relaxed">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {SECURITY_ITEMS.map((item, i) => (
              <div
                key={i}
                className="p-6 rounded-2xl bg-white/8 border border-white/10 hover:bg-white/12 hover:-translate-y-1 transition-all duration-200 group animate-fade-in-up"
                style={{ animationDelay: `${i * 0.07}s` }}
              >
                <div className="h-10 w-10 rounded-xl bg-emerald-500/20 text-emerald-300 flex items-center justify-center mb-4 group-hover:bg-emerald-500/30 transition-colors">
                  {item.icon}
                </div>
                <h3 className="text-base font-bold text-white mb-2">{item.title}</h3>
                <p className="text-sm text-emerald-200 leading-relaxed">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container-wide">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {METRICS.map((m, i) => (
              ...
              >
                <div className="h-12 w-12 rounded-xl bg-white/10 text-emerald-300 flex items-center justify-center mx-auto mb-4 group-hover:bg-white/20 transition-colors">
                  {m.icon}
                </div>
                <CounterNumber value={m.value} />
                <p className="text-emerald-200 text-xs font-medium mt-2 leading-tight">{m.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ============ CONTATO ============ */}
      <section id="contato" className="section-padding bg-[#F8FAFC]">
        <div className="container-wide">
            ...
            <div className="card p-8">
              {contactSent ? (
                <div className="text-center py-8">
                  ...
                  <h3 className="text-lg font-bold text-[#0F172A] mb-2">Mensagem enviada!</h3>
        ...
        <div className="container-wide py-12">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
            ...
            <div className="lg:col-span-2">
              ...
              <div className="flex items-center gap-2">
                <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                <span className="text-xs text-emerald-400 font-medium">Sistema operacional — 99,9% uptime</span>
              </div>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-2.5 text-sm">
                {['Comercial', 'Fiscal (GTA/NF-e)', 'Financeiro', 'Logística', 'Relatórios'].map((l) => (
                  <li key={l}>
                    <a href="#modulos" className="hover:text-emerald-400 transition-colors">{l}</a>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="border-t border-white/10 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 flex-wrap justify-center">
              {['SEFAZ', 'LGPD', 'SSL-256', 'ISO 27001'].map((tag) => (
                <span key={tag} className="flex items-center gap-1 text-[10px] font-semibold text-emerald-400 bg-emerald-900/40 px-2 py-0.5 rounded-full">
